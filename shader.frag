...
uniform vec3 u_randBrightness; // how to make density/resolution dynamic
...
void main() {
    ...
    float brightness = dot(simColor.rgb, u_randBrightness); // Grayscale brightness
    //float brightness = (simColor.r + simColor.g + simColor.b) / 3.0;
    ...
    if (u_backgroundColorMode == 0) {
        gl_FragColor = mix(vec4(simColor.rgb, 1.0), finalColor, 1.0-charColor.a);
    } else {
